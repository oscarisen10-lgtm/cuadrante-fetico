    ...
    const loop = (now) => {
      ...
      if (st.t > st.nextSpawn) {
        ...
        itemsRef.current.push({
          ...
          vx: (50 - x) * 0.0011 + (Math.random() - 0.5) * 0.02,
          vy: -(0.072 + Math.random() * 0.022),
      ...
      if (b.down) {
        ...
        if (speed > 0.8) {
          for (const it of itemsRef.current) {
            ...
            if (Math.abs(it.x - b.x) < 10 && Math.abs(it.y - b.y) < 10) {
              it.sliced = true;
  ...
  return (
    <div className="flex-1 flex flex-col px-5 pb-5 select-none">
      <div className="flex items-center justify-center gap-3 mb-3">
        ...
        <span className="bg-black/30 text-lime-400 font-black px-4 py-1.5 rounded-full text-sm">🔪 {score}</span>
      </div>
      <div
        ref={arenaRef}
      ...
      >
        {itemsRef.current.map(it => (
          <div key={it.id} className="absolute text-4xl -ml-5 -mt-5 pointer-events-none" style={{ left: `${it.x}%`, top: `${it.y}%` }}>{it.emoji}</div>
        ))}
